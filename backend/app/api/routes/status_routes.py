"""
Status endpoints for monitoring system health with enhanced security.
"""
import time
import os
import psutil
...
async def health_check(request: Request, response: Response):
    """
    Health check endpoint for monitoring with enhanced metrics.
...
async def metrics(request: Request, response: Response):
    ...
    cache_stats = {
        "cache_size": len(response_cache.cache),
        "cache_keys": len(response_cache.access_times)
    }
...
async def readiness_check(request: Request):
    ...
    """
    # Get initialization status from services
