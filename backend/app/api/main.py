...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            # Determine environment to set appropriate Content-Security-Policy.
...
class RequestSizeMiddleware(BaseHTTPMiddleware):
    ...
    It checks the 'content-length' header of incoming HTTP requests and returns a 413 error
    response if the request body size exceeds the configured limit.
    """
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if content_length is not None and int(content_length) > self.max_content_length:
                # Return error response if request body is too large.
                return Response(
                    status_code=413,
...
class ValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            # Check if any suspicious pattern is in the path.
            ...
            # Proceed with request processing if no issues.
            return await call_next(request)
        except Exception as e:
...
def create_app() -> FastAPI:
    ...
    """
    try:
        # Initialize the FastAPI app with metadata and custom docs URLs.
        app = FastAPI(
        ...
        # Initialize middlewares for security, validation, and request size checks.
        _init_middlewares(app)
        # Initialize additional rate limiting if necessary.
        _init_rate_limiting(app)

        # Add caching middleware for a set of paths to improve performance.
        app.add_middleware(CacheMiddleware,
                           paths=["/ai", "/ml", "/batch", "/pdf", "/status", "/help", "/readiness", "/metrics", "/health"],
                           ttl=600)

        # Include the routers for different API modules.
        ...
        def custom_openapi():
            ...
            # Generate the OpenAPI schema based on current routes.
        ...
        async def add_process_time_header(request: Request, call_next):
            ...
            # Set custom headers with processing time and request ID.
        ...
        async def startup_event():
            ...
            try:
                ...
                # Log startup errors but allow application to continue.
        ...
        async def shutdown_event():
            ...
            try:
                # Attempt asynchronous shutdown of initialization service.
        ...
        # Register atexit handler to ensure retention manager is shutdown on exit.
        atexit.register(lambda: retention_manager.shutdown())
        log_info("[OK] API application created and configured successfully")
        return app
    except Exception as e:
        # Log and re-raise errors during application creation.
