...
class GeminiEntityDetector(BaseEntityDetector):
    ...
    async def detect_sensitive_data_async(
        ...
        if requested_entities is None:
            log_warning(
                "[Gemini] No specific entities requested, returning empty results"
            )
        ...
        try:
            ...
            validated_entities = validate_gemini_requested_entities(
        ...
        if not validated_entities:
            log_warning(
                "[Gemini] No valid entities remain after filtering, returning empty results"
            )
            record_keeper.record_processing(
                operation_type=operation_type,
                document_type=document_type,
        ...
        try:
            ...
            pages, page_texts_and_mappings, redaction_mapping = (
                self._prepare_pages_and_mappings(minimized_data)
            )
            ...
            combined_entities = await self._process_all_pages_in_parallel(
                pages, page_texts_and_mappings, validated_entities, redaction_mapping
            )
    ...
    def _prepare_pages_and_mappings(
        ...
        List[Dict[str, Any]],
        Dict[int, Tuple[str, List[Tuple[Dict[str, Any], int, int]]]],
        ...
        page_texts_and_mappings: Dict[
            int, Tuple[str, List[Tuple[Dict[str, Any], int, int]]]
        ] = {}
        ...
        for pg in pages:
            ...
            try:
                ...
                reconstructed_text, text_mapping = (
                    TextUtils.reconstruct_text_and_mapping(pg_words)
                )
    ...
    async def _process_all_pages_in_parallel(
        ...
        log_info(
            "[OK] Processing {0} pages with Gemini API in parallel".format(len(pages))
        )
        ...
        page_results = await ParallelProcessingCore.process_pages_in_parallel(
            ...
            lambda pg: self._process_single_page(
                pg, page_texts_and_mappings, requested_entities
            ),
    ...
    async def _process_single_page(
        ...
        processed_text = await gemini_usage_manager.manage_page_processing(
            local_full_text, requested_entities, local_page_number
        )
        # If usage management blocks processing, log a warning and return empty results.
        if processed_text is None:
            log_warning(
                "[WARNING] Page {0} processing blocked by usage manager".format(
                    local_page_number
                )
            )
            return {"page": local_page_number, "sensitive": []}, []
        ...
        log_info(
            "[OK] Processing text with Gemini API on page {0}".format(local_page_number)
        )
        try:
            ...
            async with api_lock.acquire_timeout(timeout=45.0):
                ...
                response = await self.gemini_helper.process_text(
                    processed_text, requested_entities
                )
        finally:
        ...
        if response is None:
            log_warning(
                "[WARNING] Gemini API processing failed on page {0}".format(
                    local_page_number
                )
            )
            return {"page": local_page_number, "sensitive": []}, []
        ...
        log_info(
            "[OK] Found {0} entities on page {1}".format(
                detected_entity_count, local_page_number
            )
        )
    ...
    def _extract_entities_from_response(
        self, api_response: Dict[str, Any], page_full_text: str
    ) -> List[Dict[str, Any]]:
        """
        Extract entities from the Gemini API response.
        ...
        try:
            ...
            SecurityAwareErrorHandler.log_processing_error(
                exc, "gemini_response_extraction"
            )
    ...
    def _maybe_add_original_text(
        entity: Dict[str, Any], page_full_text: str
    ) -> Dict[str, Any]:
        """
        ...
        if "original_text" not in entity and "start" in entity and "end" in entity:
            try:
                ...
                log_warning(
                    "[WARNING] Could not extract original_text from entity indices"
                )
