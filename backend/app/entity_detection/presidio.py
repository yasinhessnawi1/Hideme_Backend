"""
Presidio entity detection implementation with robust async processing support.
"""
import asyncio
import os
import threading
...
class PresidioEntityDetector(BaseEntityDetector):
    ...
    with robust async processing support.
    """
    ...
    def detect_sensitive_data(self, extracted_data: Dict[str, Any], requested_entities: Optional[List[str]] = None) -> \
        ...
        try:
            ...
            try:
                return loop.run_until_complete(self.detect_sensitive_data_async(extracted_data, requested_entities))
            finally:
                loop.close()
    ...
    async def detect_sensitive_data_async(self, extracted_data: Dict[str, Any],
                                                  requested_entities: Optional[List[str]] = None) -> Tuple[
        ...
        async def process_page(page):
            ...
            try:
                ...
                page_results = await asyncio.to_thread(self._analyze_text, full_text, DEFAULT_LANGUAGE,
                                                       requested_entities)
                ...
                logger.error(f"[ERROR] Error processing page {page.get('page', 'unknown')}: {e}")
        ...
        for page_number, (page_redaction_info, processed_entities) in page_results:
            if page_redaction_info:
                ...
                page_text = next((TextUtils.reconstruct_text_and_mapping(page['words'])[0] for page in pages if
                                  page.get('page') == page_number), "")
        ...
        anonymized_text = await asyncio.to_thread(self._anonymize_text, "\n".join(anonymized_texts),
                                                  combined_results)
        ...
        log_info(
            f"[PERF] Presidio processing summary: {entity_count} entities across {len(pages)} pages in {total_time:.2f}s")
    ...
    def _analyze_text(self, text: str, language: str, entities: List[str]) -> List[RecognizerResult]:
        ...
        with self.analyzer_lock:
            try:
                ...
                logger.error(f"[ERROR] Error during Presidio analysis: {e}")
                return []
    ...
    def _anonymize_text(self, text: str, entities: List[Dict[str, Any]]) -> str:
        ...
        for entity in entities:
            try:
                recognizer_results.append(
                    RecognizerResult(entity_type=entity["entity_type"], start=entity["start"], end=entity["end"],
        ...
        try:
            ...
            logger.error(f"[ERROR] Error during anonymization: {e}")
            return text
    ...
    def _convert_to_entity_dict(self, entity: Union[RecognizerResult, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if isinstance(entity, RecognizerResult):
            ...
            return {"entity_type": entity.entity_type, "start": entity.start, "end": entity.end,
                    "score": float(entity.score)  # Ensure score is a float
                    }
        elif isinstance(entity, dict):
            # For dictionary entities, ensure required fields
            entity_dict = {"entity_type": entity.get("entity_type", "UNKNOWN"), "start": entity.get("start", 0),
                           "end": entity.get("end", 0), "score": float(entity.get("score", 0.5))}
            ...
            try:
                return {"entity_type": getattr(entity, "entity_type", "UNKNOWN"),
                        "start": getattr(entity, "start", 0), "end": getattr(entity, "end", 0),
                        "score": float(getattr(entity, "score", 0.5))}
            except Exception as e:
                logger.error(f"[ERROR] Failed to convert entity: {e}")
