...
class GenericEntityDetector(BaseEntityDetector):
    ...
    _shared_instance_lock = TimeoutLock(
        name="generic_shared_instance_lock", priority=LockPriority.HIGH, timeout=600.0
    )
    _model_lock = TimeoutLock(
        name="generic_model_lock", priority=LockPriority.HIGH, timeout=600.0
    )
    ...
    def __init__(
        ...
        if not ENGINE_LIB_AVAILABLE:
            log_error(
                f"[ERROR] {self.ENGINE_NAME} package not installed. Please install with 'pip install gliner'"
            )
        ...
        if self._model_analyzer_lock is None:
            try:
                ...
                SecurityAwareErrorHandler.log_processing_error(
                    exc, "model_analyzer_lock_init", self._model_dir_path
                )
    ...
    def _initialize_model(self) -> None:
        ...
        cache_key = (
            ...
            tuple(sorted(self._default_entities)),
        )
        ...
        if not acquired:
            log_error(
                f"[{self.ENGINE_NAME}] Timeout acquiring model lock for initialization"
            )
            return
        try:
    ...
    def _try_load_model_from_cache(self, cache_key) -> bool:
        ...
        if not acquired:
            ...
            log_warning(
                f"[{self.ENGINE_NAME}] Timeout acquiring model lock for cache check"
            )
            return False
        try:
    ...
    def _handle_concurrent_initialization(self, cache_key) -> bool:
        ...
        if self.__class__._global_initializing:
            ...
            log_info(
                f"[{self.ENGINE_NAME}] Another thread is already initializing the model"
            )
    ...
    def _perform_model_initialization(
        self, cache_key, start_time: float, max_retries: int = 2
    ) -> None:
        """
        ...
        for attempt_index in range(max_retries):
            ...
            log_info(
                f"[{self.ENGINE_NAME}] Attempt {attempt_index + 1}/{max_retries} for model initialization."
            )
    ...
    def _attempt_model_init(self, local_exists: bool, attempt_index: int) -> bool:
        ...
        try:
            ...
            if local_exists:
                log_info(
                    f"[OK] Attempting to load model from local directory: {self._model_dir_path}"
                )
            ...
            if not local_exists:
                log_info(
                    f"[OK] Downloading {self.ENGINE_NAME} model: {self._model_name}"
                )
    ...
    def _load_model_from_cache_and_log(self, cache_key: tuple, reason: str) -> bool:
        ...
        log_info(
            f"[{self.ENGINE_NAME}] Loaded model from cache {reason}in {self._initialization_time:.2f}s"
        )
    ...
    def _fix_config_file_discrepancy(self) -> None:
        ...
        try:
            ...
            engine_config_path = os.path.join(
                self._model_dir_path, self.CONFIG_FILE_NAME
            )
            ...
            if os.path.exists(engine_config_path) and not os.path.exists(
                common_config_path
            ):
                # Log the start of the copying process.
                log_info(
                    f"[{self.ENGINE_NAME}] Creating config.json from {self.CONFIG_FILE_NAME}"
                )
    ...
    def _check_local_model_exists(self) -> bool:
        ...
        if not os.path.exists(self._model_dir_path):
            ...
            log_warning(
                f"[{self.ENGINE_NAME}] Model directory does not exist: {self._model_dir_path}"
            )
            return False
        ...
        if not os.path.isdir(self._model_dir_path):
            ...
            log_warning(
                f"[{self.ENGINE_NAME}] Model path is not a directory: {self._model_dir_path}"
            )
            return False
        ...
        has_config = any(
            os.path.exists(os.path.join(self._model_dir_path, f)) for f in config_files
        )
        # If no configuration file is found:
        if not has_config:
            # Log a warning and return False.
            log_warning(
                f"[{self.ENGINE_NAME}] No configuration file found in model directory"
            )
            return False
        ...
        missing_files = [
            ...
            if not os.path.exists(os.path.join(self._model_dir_path, f))
        ]
        # If required files are missing:
        if missing_files:
            # Log a warning mentioning the missing files.
            log_warning(
                f"[{self.ENGINE_NAME}] Missing required model files: {missing_files}"
            )
            return False
        # Log that the model directory is valid.
        log_info(
            f"[{self.ENGINE_NAME}] Found valid model directory at {self._model_dir_path}"
        )
        return True
    ...
    def _save_model_to_directory(self, model_dir: str) -> bool:
        ...
        try:
            ...
            if not self.model:
                log_error(
                    f"[ERROR] Cannot save model - {self.ENGINE_NAME} model not initialized"
                )
                return False
            ...
            if hasattr(self.model, "save_pretrained"):
                ...
                log_info(
                    f"[OK] Used {self.ENGINE_NAME}'s native save_pretrained method"
                )
            else:
            ...
            SecurityAwareErrorHandler.log_processing_error(
                exc, f"{self.ENGINE_NAME.lower()}_model_save", model_dir
            )
            return False
    ...
    def _load_local_model(self, model_dir: str) -> bool:
        ...
        try:
            ...
            log_info(
                f"[OK] Loading {self.ENGINE_NAME} model from directory: {model_dir}"
            )
            ...
            try:
                ...
                if not os.path.exists(config_path):
                    log_warning(
                        f"[{self.ENGINE_NAME}] config.json still doesn't exist after fix attempt"
                    )
                ...
                log_info(
                    f"[OK] Loaded model using {self.ENGINE_NAME}'s native from_pretrained method"
                )
                return True
            ...
            SecurityAwareErrorHandler.log_processing_error(
                exc, f"{self.ENGINE_NAME.lower()}_model_load", model_dir
            )
            return False
    ...
    def _attempt_explicit_config_load(self, model_dir: str) -> bool:
        ...
        try:
            ...
            chosen_config = (
                config_path if os.path.exists(config_path) else engine_config_path
            )
            ...
            if os.path.exists(chosen_config):
                ...
                log_info(
                    f"[{self.ENGINE_NAME}] Trying to load with explicit config path: {chosen_config}"
                )
                try:
                    ...
                    log_info(
                        f"[OK] Loaded model with explicit config for {self.ENGINE_NAME}"
                    )
                    return True
                except ImportError:
                    # Log a warning if AutoConfig could not be imported.
                    log_warning(
                        f"[WARNING] Could not import AutoConfig from transformers for {self.ENGINE_NAME}"
                    )
    ...
    async def detect_sensitive_data_async(
        ...
        try:
            ...
            if requested_entities is not None:
                ...
                validated_entities = self.validate_requested_entities(
                    requested_entities_json
                )
            else:
        ...
        if not validated_entities:
            ...
            log_warning(
                f"[{self.ENGINE_NAME}] No valid entities remain after filtering, returning empty results"
            )
            record_keeper.record_processing(
                operation_type=f"{self.ENGINE_NAME.lower()}_detection",
                document_type="document",
        ...
        try:
            ...
            page_results = await self._process_pages_async(
                pages, page_texts_and_mappings, validated_entities
            )
            # Initialize an empty list to collect combined results.
            combined_results = []
            # Iterate over the results for each page.
            for local_page_number, (
                local_page_redaction_info,
                local_processed_entities,
            ) in page_results:
            ...
            SecurityAwareErrorHandler.log_processing_error(
                exc, f"{self.ENGINE_NAME.lower()}_detection", ""
            )
    ...
    async def _process_pages_async(
        ...
        async def process_page_async(
            ...
            try:
                return await asyncio.wait_for(
                    self._process_single_page_async(
                        local_page_data, page_texts_and_mappings, requested_entities
                    ),
                    timeout=600.0,
                )
            # Handle a timeout error specifically.
                ...
                log_warning(
                    f"[WARNING] Timeout processing page {local_page_number} with {self.ENGINE_NAME}"
                )
        ...
        return await ParallelProcessingCore.process_pages_in_parallel(
            pages, process_page_async
        )
    ...
    async def _process_single_page_async(
        ...
        log_info(
            f"[OK] Processing text with {self.ENGINE_NAME} on page {local_page_number}"
        )
        # If the model is not loaded, log a warning and skip this page.
        if not self.model:
            log_warning(
                f"[WARNING] {self.ENGINE_NAME} model not available, skipping page {local_page_number}"
            )
            return {"page": local_page_number, "sensitive": []}, []
        # Process the text using the model in a separate thread.
        page_entities = await asyncio.to_thread(
            self._process_text, full_text, requested_entities
        )
        # If no entities are detected, log the event and return empty results.
        if not page_entities:
            log_info(
                f"[OK] No entities detected by {self.ENGINE_NAME} on page {local_page_number}"
            )
            return {"page": local_page_number, "sensitive": []}, []
        ...
        processed_entities, local_page_redaction_info = (
            await ParallelProcessingCore.process_entities_in_parallel(
        ...
        log_sensitive_operation(
            f"{self.ENGINE_NAME} Page Detection",
        ...
        for page in pages:
            ...
            if not words:
                redaction_mapping["pages"].append(
                    {"page": local_page_number, "sensitive": []}
                )
                continue
    ...
    def _process_text(
        ...
        cached = GLiNERHelper.get_cached_result(
            key, cache_namespace=self.CACHE_NAMESPACE
        )
        ...
        for i in range(0, len(paragraphs), batch_size):
            ...
            batch_results = self._process_paragraph_batch(
                batch_paragraphs, text, requested_entities
            )
        ...
        GLiNERHelper.set_cached_result(
            key, filtered_entities, cache_namespace=self.CACHE_NAMESPACE
        )
        # Return the filtered list of entities.
        return filtered_entities

    def _process_paragraph_batch(
        self, paragraphs: List[str], full_text: str, requested_entities: List[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for paragraph in paragraphs:
            ...
            for group_text in sentence_groups:
                ...
                group_entities = self._extract_entities_for_group(
                    group_text, requested_entities, absolute_offset
                )
    ...
    def _extract_entities_for_group(
        self, group_text: str, requested_entities: List[str], absolute_offset: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            group_entities = self.model.predict_entities(
                group_text, requested_entities, threshold=0.40
            )
    ...
    def _filter_norwegian_pronouns(
        ...
        for ent in entities:
            ...
            if etype in {"person", "per", "PERSON-H"}:
                ...
                if (
                    len(text_words) == 1
                    and text_words[0].lower() in self.norwegian_pronouns
                ):
                    log_info(
                        "[FILTER] Removed Norwegian pronoun incorrectly detected as person"
                    )
                    continue
                # If the text contains multiple words and all are pronouns, skip the entity.
                if len(text_words) > 1 and all(
                    w.lower() in self.norwegian_pronouns for w in text_words
                ):
                    log_info(
                        "[FILTER] Removed compound Norwegian pronoun incorrectly detected as person"
                    )
                    continue
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "idle_time_readable": (
                f"{(time.time() - self._last_used):.2f} seconds"
                if self._last_used
                else "N/A"
            ),
            ...
            "average_processing_time": (
                self.total_processing_time / self._total_calls
            ...
            "model_directory_exists": (
                os.path.isdir(self._model_dir_path) if self._model_dir_path else False
            ),
            "model_files": (
                os.listdir(self._model_dir_path)
                if self._model_dir_path and os.path.isdir(self._model_dir_path)
                else []
            ),
            "config_files": {
                self.CONFIG_FILE_NAME: (
                    os.path.exists(
                        os.path.join(self._model_dir_path, self.CONFIG_FILE_NAME)
                    )
                    if self._model_dir_path
                    else False
                )
            },
