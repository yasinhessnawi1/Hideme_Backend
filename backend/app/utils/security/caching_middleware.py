...
synchronization using a basic lock (acquire/release), and TTL-based cache expiration to
...
class ResponseCache:
    ...
    Allows concurrent reads (without locking) for high-traffic paths and uses exclusive writes
    to maximize throughput while maintaining data consistency.
    """
    def __init__(self, max_size: int = 1000, default_ttl: int = 300):
        ...
        # Use TimeoutLock for synchronization; previously set as low priority for caching.
        self.rwlock = TimeoutLock("response_cache_lock", priority=LockPriority.LOW)
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        if value is not None:
            ...
            if current_time < expiry:
                # Optionally update access time (this update is lock-free; slight race conditions are acceptable)
                try:
    ...
    def set(self, key: str, value: Any, ttl: Optional[int] = None, content_hash: Optional[str] = None) -> bool:
        """
        Set a value in the cache with exclusive lock synchronization.
        ...
        """
        try:
            acquired = self.rwlock.acquire(timeout=5.0)
            if not acquired:
    ...
    def delete(self, key: str) -> bool:
        """
        Delete a value from the cache with exclusive lock synchronization.
        ...
        """
        try:
            acquired = self.rwlock.acquire(timeout=3.0)
            if not acquired:
    ...
    def clear(self) -> bool:
        """
        Clear the entire cache with exclusive lock synchronization.

        Returns:
            True if cleared successfully, False otherwise.
        """
        try:
            acquired = self.rwlock.acquire(timeout=10.0)
            if not acquired:
    ...
    def cleanup_expired(self) -> int:
        """
        Clean up expired cache entries with exclusive lock.
...
def invalidate_cache(path_prefix: Optional[str] = None) -> None:
    ...
    if path_prefix is None:
        ...
        keys_to_remove = []
        try:
            acquired = response_cache.rwlock.acquire(timeout=3.0)
            if acquired:
                try:
