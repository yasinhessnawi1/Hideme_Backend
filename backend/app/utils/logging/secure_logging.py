...
def log_sensitive_operation(
    operation_name: str, entity_count: int, processing_time: float, **metadata
) -> None:
    """
    ...
    log_info(
        f"[SENSITIVE] {operation_name}: processed {entity_count} entities in {processing_time:.2f}s"
    )
    ...
    if metadata:
        ...
        sanitized_metadata = {
            k: v for k, v in metadata.items() if k not in sensitive_keys
        }
...
def log_batch_operation(
    operation_name: str, total_files: int, successful_files: int, processing_time: float
) -> None:
    """
    ...
    log_info(
        f"[BATCH] {operation_name}: {successful_files}/{total_files} files in {processing_time:.2f}s"
    )
