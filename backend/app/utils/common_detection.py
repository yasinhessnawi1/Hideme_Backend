...
With enhanced synchronization features using timeout locks to prevent deadlocks.
"""
import time
import asyncio
...
# Shared resource lock for detection processing
...
async def process_common_detection(
    ...
    Enhanced with proper lock management and timeout handling.
    ...
    try:
        ...
        try:
            ...
            async with _detection_resource_lock.acquire_timeout(timeout=15.0) as acquired:
                if not acquired:
                    log_warning(f"[DETECTION] Failed to acquire lock for {detector_type} detector [operation_id={operation_id}]")
                    # Continue with a direct detector initialization as fallback
                    detector = get_detector_func(entity_list)
                    log_info(f"[DETECTION] Using detector without lock acquisition [operation_id={operation_id}]")
                else:
            ...
            log_error(f"[DETECTION] Error acquiring detector lock: {str(lock_error)} [operation_id={operation_id}]")
        ...
        try:
            ...
            error_response, status_code = SecurityAwareErrorHandler.create_api_error_response(
                TimeoutError(f"Detection operation timed out after 60.0 seconds"),
