...
class TestPdfRoutes:
    ...
    @pytest.mark.asyncio
    @patch("backend.app.services.document_redact_service.DocumentRedactionService.redact")
    async def test_pdf_redact_success(self, mock_redact):
        mock_redact.return_value = Response(content=b"Redacted PDF content", media_type="application/pdf")
    ...
    @pytest.mark.asyncio
    @patch("backend.app.services.document_redact_service.DocumentRedactionService.redact")
    @patch("backend.app.api.routes.pdf_routes.SecurityAwareErrorHandler.handle_safe_error")
    ...
    async def test_pdf_redact_invalid_mapping(self, mock_redact):
        ...
        mock_redact.side_effect = HTTPException(status_code=400, detail="Invalid redaction mapping")
    ...
    @pytest.mark.asyncio
    @patch("backend.app.services.document_extract_service.DocumentExtractService.extract")
    async def test_pdf_extract_success(self, mock_extract):
        ...
        assert response.json() == {"text": "Extracted text", "positions": [[0, 0], [10, 10]]}
    ...
    @patch("backend.app.services.document_extract_service.DocumentExtractService.extract")
    @patch("backend.app.api.routes.pdf_routes.SecurityAwareErrorHandler.handle_safe_error")
    async def test_pdf_extract_internal_error(self, mock_handle_safe_error, mock_extract):
        response_cache.clear()
    ...
    async def test_pdf_extract_invalid_file(self, mock_extract):
        ...
        mock_extract.side_effect = HTTPException(status_code=400, detail="Invalid file format")
