...
class TestBatchEncryptedRoutes:
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    @patch("backend.app.api.routes.batch_routes.BatchDetectService.detect_entities_in_files", new_callable=AsyncMock)
    def test_batch_detect_success(self, mock_detect, mock_prepare,
                                  encrypted_file, enc_req_entities, enc_remove_words):
        ...
        inner = json.loads(decrypt_data(base64.urlsafe_b64decode(cipher_b64), DUMMY_API_KEY))
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    def test_batch_detect_bad_threshold(self, mock_prepare, mock_validate,
                                        encrypted_file, enc_req_entities, enc_remove_words):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
    ...
    @patch("backend.app.api.routes.batch_routes.BatchDetectService.detect_entities_in_files",
           side_effect=Exception("oops"))
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    def test_batch_detect_internal_error(self, mock_prepare, mock_detect, mock_handle,
                                         encrypted_file, enc_req_entities, enc_remove_words):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
            {"requested_entities": "X", "remove_words": "Y"},
        ...
        resp = client.post(
            "/batch/detect",
            files=[("files", encrypted_file)],
            data={"threshold": "0.3", "requested_entities": b64_req, "remove_words": b64_rem},
            headers={"session-key": "s", "api-key-id": "id"}
        )
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    @patch("backend.app.api.routes.batch_routes.BatchDetectService.detect_entities_in_files", new_callable=AsyncMock)
    def test_batch_hybrid_success(self, mock_detect, mock_prepare,
                                  encrypted_file, enc_req_entities, enc_remove_words):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
        ...
        inner = json.loads(
            decrypt_data(base64.urlsafe_b64decode(resp.json()["encrypted_data"]), DUMMY_API_KEY)
        )
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    @patch("backend.app.api.routes.batch_routes.BatchSearchService.batch_search_text", new_callable=AsyncMock)
    def test_batch_search_success(self, mock_search, mock_prepare,
                                  encrypted_file, enc_search_terms):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
            {"search_terms": "findme"},
            DUMMY_API_KEY
    ...
    @patch("backend.app.api.routes.batch_routes.BatchSearchService.batch_search_text",
           side_effect=Exception("explode"))
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    def test_batch_search_internal_error(self, mock_prepare, mock_search, mock_handle,
                                         encrypted_file, enc_search_terms):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
            {"search_terms": "findme"},
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    @patch("backend.app.api.routes.batch_routes.BatchSearchService.find_words_by_bbox", new_callable=AsyncMock)
    def test_batch_find_words_success(self, mock_find, mock_prepare,
                                      encrypted_file, enc_bbox):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    def test_batch_find_words_bad_bbox_format(self, mock_prepare, encrypted_file, enc_bbox):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    def test_batch_find_words_missing_keys(self, mock_prepare, encrypted_file, enc_bbox):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    @patch("backend.app.api.routes.batch_routes.BatchRedactService.batch_redact_documents", new_callable=AsyncMock)
    def test_batch_redact_success_encrypted(self, mock_redact, mock_prepare,
                                            encrypted_file, enc_redaction_map):
        payload = json.dumps({"ok": True}).encode()

        mock_redact.return_value = StarletteResponse(payload, media_type="application/octet-stream")
    ...
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    @patch("backend.app.api.routes.batch_routes.BatchRedactService.batch_redact_documents", new_callable=AsyncMock)
    def test_batch_redact_success_public(self, mock_redact, mock_prepare, encrypted_file, enc_redaction_map):
    ...
    @patch("backend.app.api.routes.batch_routes.BatchRedactService.batch_redact_documents",
           side_effect=Exception("fail"))
    @patch("backend.app.api.routes.batch_routes.session_manager.prepare_inputs", new_callable=AsyncMock)
    def test_batch_redact_internal_error(self, mock_prepare, mock_redact, mock_handle,
                                         encrypted_file, enc_redaction_map):
        mock_prepare.return_value = (
            [UploadFile(filename="test.pdf", file=io.BytesIO(b"PDFDATA"))],
            {"redaction_mappings": json.dumps({"foo": "bar"})},
