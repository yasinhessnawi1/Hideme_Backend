...
class TestDocumentRedactionService(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("backend.app.services.document_redact_service.memory_monitor.get_memory_stats", return_value={"current_usage": 50.0, "peak_usage": 75.0})
    ...
    @patch("backend.app.services.document_redact_service.read_and_validate_file", return_value=(None, JSONResponse(content={"error": "invalid"}), 0))
    ...
    @patch("backend.app.services.document_redact_service.read_and_validate_file", return_value=(b"valid-bytes", None, 0))
    ...
    def test_parse_redaction_mapping_invalid(self):
        result, count, err = DocumentRedactionService._parse_redaction_mapping("not-json", "file.pdf", "op2")
        self.assertEqual(count, 0)
        self.assertIsInstance(err, JSONResponse)

    def test_parse_redaction_mapping_none(self):
        result, count, err = DocumentRedactionService._parse_redaction_mapping(None, "file.pdf", "op3")
        self.assertEqual(count, 0)
