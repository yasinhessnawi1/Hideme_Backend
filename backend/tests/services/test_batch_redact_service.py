...
class TestBatchRedactService(unittest.IsolatedAsyncioTestCase):
    ...
    def test_parse_redaction_mappings_single_file(self):
        ...
        json_str = json.dumps(
            {"redaction_mapping": {"pages": []}, "file_info": {"filename": "test.pdf"}}
        )
    ...
    def test_compute_redaction_summary_values(self):
        ...
        success, fail, total, time, summary = (
            BatchRedactService._compute_redaction_summary(metadata, mapping, 0.0, "id")
        )
    ...
    def test_build_streaming_response_headers(self):
        ...
        resp = BatchRedactService.build_streaming_response(
            "some.zip", summary, "batch001"
        )
    ...
    def test_prepare_redaction_items_basic(self):
        ...
        meta = [{"mapping": {"pages": []}, "safe_name": "safe.pdf"}]
    ...
    @patch(
        "backend.app.services.batch_redact_service.read_and_validate_file",
        return_value=(b"%PDF", None, 0.1),
    )
    @patch(
        "backend.app.services.batch_redact_service.sanitize_filename",
    ...
    async def test_prepare_files_for_redaction_missing_mapping(self):
        ...
        result_meta, valid_files = (
            await BatchRedactService._prepare_files_for_redaction([file], {}, "op789")
        )
    ...
    def test_create_zip_archive_basic(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = {
                "file_results": [
                    {"file": "test.pdf", "status": "success", "arcname": "test.pdf"}
                ]
            }
    ...
    @patch(
        "backend.app.services.batch_redact_service.PDFRedactionService.apply_redactions",
        return_value="/tmp/redacted.pdf",
    )
    @patch("backend.app.services.batch_redact_service.PDFRedactionService.close")
    @patch(
        "backend.app.services.batch_redact_service.ParallelProcessingCore.process_in_parallel"
    )
    async def test_process_redaction_items_success(
        self, mock_parallel, mock_close, mock_apply
    ):
    ...
    async def test_too_many_files(self):
        ...
        result = await BatchRedactService.batch_redact_documents(
            files=[DummyUploadFile(), DummyUploadFile()], redaction_mappings="{}"
        )
    ...
    async def test_invalid_redaction_mappings(self):
        ...
        result = await BatchRedactService.batch_redact_documents(
            files=[DummyUploadFile()], redaction_mappings="invalid"
        )
    ...
    @patch(
        "backend.app.services.batch_redact_service.SecureTempFileManager.create_secure_temp_dir_async",
        new_callable=AsyncMock,
    )
    @patch(
        "backend.app.services.batch_redact_service.SecureTempFileManager.secure_delete_directory"
    )
    @patch(
        "backend.app.services.batch_redact_service.BatchRedactService._prepare_files_for_redaction",
        return_value=([], []),
    )
    ...
    @patch(
        "backend.app.services.batch_redact_service.SecureTempFileManager.create_secure_temp_file_async",
        new_callable=AsyncMock,
    )
    @patch(
        "backend.app.services.batch_redact_service.SecureTempFileManager.create_secure_temp_dir_async",
    ...
    @patch(
        "backend.app.services.batch_redact_service.BatchRedactService._process_redaction_items",
        new_callable=AsyncMock,
    )
    @patch(
        "backend.app.services.batch_redact_service.BatchRedactService._create_zip_archive"
    )
    ...
    async def test_successful_batch_redact_documents(
        ...
        response = await BatchRedactService.batch_redact_documents(
            files=[DummyUploadFile()], redaction_mappings=mappings
        )
    ...
    @patch(
        "backend.app.services.batch_redact_service.PDFRedactionService",
        new=DummyPDFRedactionService,
    )
    async def test_process_redaction_item_success(self):

        item = (0, b"pdf", {"pages": [{"sensitive": ["test"]}]}, "/tmp")

        result = await BatchRedactService._process_redaction_items(
            [item], max_workers=1, remove_images=False
        )
