...
class TestMashinLearningService(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('backend.app.services.machine_learning_service.BaseDetectionService.prepare_detection_context',
           return_value=(None, None, None, None, HTTPException(status_code=500, detail={"err": True})))
    ...
    @patch('backend.app.services.machine_learning_service.MashinLearningService.perform_detection',
           new_callable=AsyncMock)
    async def test_detect_none_result(self, mock_detect, mock_init, mock_min, mock_ctx, mock_safe_error):
