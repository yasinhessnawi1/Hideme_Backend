...
class TestBaseDetectionService(unittest.IsolatedAsyncioTestCase):
    ...
    @patch(
        "backend.app.services.base_detect_service.SecurityAwareErrorHandler.handle_safe_error"
    )
    ...
    @patch(
        "backend.app.services.base_detect_service.validate_all_engines_requested_entities"
    )
    ...
    def test_process_requested_entities_failure(
        self, mock_handle, mock_warn, mock_valid
    ):
    ...
    @patch(
        "asyncio.wait_for",
        new_callable=lambda: AsyncMock(side_effect=asyncio.TimeoutError()),
    )
    ...
    async def test_perform_detection_with_timeout(self, mock_wait):
        with warnings.catch_warnings():
            ...
            res, err = await BaseDetectionService.perform_detection(
                det, {}, [], 1, "op"
            )
    ...
    @patch.object(
        ...
        "validate_mime",
        return_value=JSONResponse(status_code=415, content={}),
    )
    async def test_prepare_detection_context_mime_error(self, mock_mime):
        file = DummyFile("x", "f")

        out = await BaseDetectionService.prepare_detection_context(
            file, None, "op", time.time()
        )
    ...
    @patch.object(
        BaseDetectionService,
        "process_requested_entities",
        return_value=(None, JSONResponse(status_code=400, content={})),
    )
    async def test_prepare_detection_context_entity_error(
        self, mock_entities, mock_mime
    ):
        file = DummyFile("a", "b")
    ...
    @patch.object(BaseDetectionService, "validate_mime", return_value=None)
    @patch.object(
        BaseDetectionService, "process_requested_entities", return_value=([], None)
    )
    @patch(
        "backend.app.services.base_detect_service.read_and_validate_file",
        new_callable=AsyncMock,
        return_value=(None, JSONResponse(status_code=500, content={}), 0),
    )
    async def test_prepare_detection_context_read_error(
        self, mock_read, mock_proc, mock_mime
    ):
        file = DummyFile("a", "b")
    ...
    @patch(
        "backend.app.services.base_detect_service.validate_file_content_async",
        new_callable=AsyncMock,
        return_value=(False, "reason", None),
    )
    async def test_prepare_detection_context_invalid_content(
        self, mock_validate_async, mock_read, mock_entities, mock_mime
    ):
        file = DummyFile("a", "b")
    ...
    @patch(
        "backend.app.services.base_detect_service.BaseDetectionService.extract_text",
        return_value=({"pages": [{"words": ["w"]}]}, None),
    )
    async def test_prepare_detection_context_success(
        self, mock_extract, mock_validate_async, mock_read, mock_entities, mock_mime
    ):
        file = DummyFile("a", "b")
    ...
    @patch(
        "backend.app.services.base_detect_service.BaseDetectionService.parse_remove_words",
        return_value=["x"],
    )
    ...
    @patch(
        "backend.app.services.base_detect_service.BaseEntityDetector.filter_by_score",
        return_value=[{"s": 1}],
    )
    def test_apply_threshold_filter(self, mock_filter):
        ents, mapping = BaseDetectionService.apply_threshold_filter(
            [{"s": 0.9}], {"pages": []}, 0.5
        )
    ...
    @patch(
        "backend.app.services.base_detect_service.BaseDetectionService.apply_threshold_filter",
        return_value=([1], {"pages": []}),
    )
    @patch(
        "backend.app.services.base_detect_service.sanitize_detection_output",
        return_value={"out": 1},
    )
    @patch(
        "backend.app.services.base_detect_service.memory_monitor.get_memory_stats",
