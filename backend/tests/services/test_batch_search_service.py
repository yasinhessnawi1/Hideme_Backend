...
class TestBatchSearchService(unittest.IsolatedAsyncioTestCase):
    ...
    @patch(
        "backend.app.services.batch_search_service.read_and_validate_file",
    ...
    async def test_read_files_with_validation_error(self, mock_validate):
        ...
        contents, metadata = await BatchSearchService._read_files_for_extraction(
            files, "op123"
        )
    ...
    @patch(
        "backend.app.services.batch_search_service.PDFSearcher.search_terms",
        new_callable=AsyncMock,
    )
    ...
    @patch(
        "backend.app.services.batch_search_service.PDFTextExtractor.extract_batch_text",
        new_callable=AsyncMock,
    )
    @patch(
        "backend.app.services.batch_search_service.BatchSearchService._read_files_for_extraction",
        new_callable=AsyncMock,
    )
    @patch(
        "backend.app.services.batch_search_service.BatchSearchService._process_single_file_result",
        new_callable=AsyncMock,
    )
    async def test_batch_search_text_success(
        self, mock_process, mock_read, mock_extract
    ):
        mock_read.return_value = (
            [b"%PDF"],
            [{"original_name": "f1", "status": "success"}],
        )

        mock_extract.return_value = [(0, {"pages": [{"words": ["test"]}]})]

        mock_process.return_value = (
            {"file": "f1", "status": "success", "results": {"match_count": 1}},
            1,
            0,
            1,
        )
    ...
    async def test_find_words_by_bbox_success(
        self, mock_extract_batch_text: AsyncMock, mock_validate: AsyncMock
    ):
        mock_validate.return_value = (b"%PDF", None, 0.1)
        mock_extract_batch_text.return_value = [(0, {"pages": [{"words": ["word"]}]})]
        ...
        with patch(
            "backend.app.services.batch_search_service.PDFSearcher",
