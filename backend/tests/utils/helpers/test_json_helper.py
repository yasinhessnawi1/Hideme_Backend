"""
Unit tests for json_helper.py module.

This test file covers the JSON helper functions with both positive
...
class TestValidateThresholdScore(unittest.TestCase):
    ...
    def test_validate_threshold_score_below_range(self):
        ...
        self.assertEqual(context.exception.detail, "Threshold must be between 0.00 and 1.00.")
...
class TestCheckAllOption(unittest.TestCase):
    ...
    @patch('backend.app.utils.helpers.json_helper.PRESIDIO_AVAILABLE_ENTITIES', ['PERSON', 'EMAIL_ADDRESS'])
    @patch('backend.app.utils.helpers.json_helper.GEMINI_AVAILABLE_ENTITIES', {'PERSON': 'person', 'EMAIL': 'email'})
    @patch('backend.app.utils.helpers.json_helper.GLINER_AVAILABLE_ENTITIES', ['PERSON', 'ORGANIZATION'])
    @patch('backend.app.utils.helpers.json_helper.HIDEME_AVAILABLE_ENTITIES', ['PERSON', 'PHONE_NUMBER'])
    ...
    def test_check_all_option_with_all_options(self, *mocks):
        result = check_all_option(["ALL_PRESIDIO", "ALL_GEMINI", "ALL_GLINER", "ALL_HIDEME"])
        expected = [
            'PERSON', 'EMAIL_ADDRESS',  # PRESIDIO
            'PERSON', 'EMAIL',  # GEMINI
            'PERSON', 'ORGANIZATION',  # GLINER
            'PERSON', 'PHONE_NUMBER'  # HIDEME
        ]
        self.assertEqual(result, expected)
...
class TestValidateAllEnginesRequestedEntities(unittest.TestCase):
    ...
    @patch('backend.app.utils.helpers.json_helper.PRESIDIO_AVAILABLE_ENTITIES', ['P1', 'P2'])
    @patch('backend.app.utils.helpers.json_helper.GEMINI_AVAILABLE_ENTITIES', {'G1': 'gemini1', 'G2': 'gemini2'})
    @patch('backend.app.utils.helpers.json_helper.GLINER_AVAILABLE_ENTITIES', ['L1', 'L2'])
    @patch('backend.app.utils.helpers.json_helper.HIDEME_AVAILABLE_ENTITIES', ['H1', 'H2'])
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    @patch('backend.app.utils.helpers.json_helper.log_info')
    def test_validate_all_engines_requested_entities_none(self, mock_log_info, mock_log_warning, *mocks):
        result = validate_all_engines_requested_entities(None)
        expected = ['G1', 'G2', 'P1', 'P2', 'L1', 'L2', 'H1', 'H2']
        self.assertEqual(result, expected)
        mock_log_warning.assert_called_once()
        self.assertIn("No specific entities requested", mock_log_warning.call_args[0][0])
    ...
    @patch('backend.app.utils.helpers.json_helper.HIDEME_AVAILABLE_ENTITIES', ['H1', 'H2'])
    @patch('backend.app.utils.helpers.json_helper.log_info')
    def test_validate_all_engines_requested_entities_valid_json(self, mock_log_info, *mocks):
...
class TestValidateGeminiRequestedEntities(unittest.TestCase):
    ...
    @patch('backend.app.utils.helpers.json_helper.log_info')
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    def test_validate_gemini_requested_entities_all_gemini(self, mock_log_warning, mock_log_info):
        # When input JSON is '["ALL_GEMINI"]', should expand to keys of GEMINI_AVAILABLE_ENTITIES.
        # We'll patch GEMINI_AVAILABLE_ENTITIES with a dummy dictionary.
        with patch('backend.app.utils.helpers.json_helper.GEMINI_AVAILABLE_ENTITIES', {'G1': 'gem1', 'G2': 'gem2'}):
    ...
    @patch('backend.app.utils.helpers.json_helper.log_info')
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    def test_validate_gemini_requested_entities_valid(self, mock_log_warning, mock_log_info):
        with patch('backend.app.utils.helpers.json_helper.GEMINI_AVAILABLE_ENTITIES', {'G1': 'gem1', 'G2': 'gem2'}):
    ...
    def test_validate_gemini_requested_entities_invalid_json(self):
        ...
        self.assertEqual(context.exception.status_code, 400)
        self.assertEqual(context.exception.detail, "Invalid JSON format in gemini requested_entities")
...
class TestValidatePresidioRequestedEntities(unittest.TestCase):
    ...
    @patch('backend.app.utils.helpers.json_helper.log_info')
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    def test_validate_presidio_requested_entities_all_presidio(self, mock_log_warning, mock_log_info):
        with patch('backend.app.utils.helpers.json_helper.PRESIDIO_AVAILABLE_ENTITIES', ['P1', 'P2']):
    ...
    @patch('backend.app.utils.helpers.json_helper.log_info')
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    def test_validate_presidio_requested_entities_valid(self, mock_log_warning, mock_log_info):
        with patch('backend.app.utils.helpers.json_helper.PRESIDIO_AVAILABLE_ENTITIES', ['P1', 'P2']):
...
class TestValidateGlinerRequestedEntities(unittest.TestCase):
    ...
    @patch('backend.app.utils.helpers.json_helper.log_info')
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    def test_validate_gliner_requested_entities_all_gliner(self, mock_log_warning, mock_log_info):
        with patch('backend.app.utils.helpers.json_helper.GLINER_AVAILABLE_ENTITIES', ['L1', 'L2']):
    ...
    @patch('backend.app.utils.helpers.json_helper.log_info')
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    def test_validate_gliner_requested_entities_valid(self, mock_log_warning, mock_log_info):
        with patch('backend.app.utils.helpers.json_helper.GLINER_AVAILABLE_ENTITIES', ['L1', 'L2']):
...
class TestValidateHidemeRequestedEntities(unittest.TestCase):
    ...
    @patch('backend.app.utils.helpers.json_helper.log_info')
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    def test_validate_hideme_requested_entities_all_hideme(self, mock_log_warning, mock_log_info):
        with patch('backend.app.utils.helpers.json_helper.HIDEME_AVAILABLE_ENTITIES', ['H1', 'H2']):
    ...
    @patch('backend.app.utils.helpers.json_helper.log_info')
    @patch('backend.app.utils.helpers.json_helper.log_warning')
    def test_validate_hideme_requested_entities_valid(self, mock_log_warning, mock_log_info):
        with patch('backend.app.utils.helpers.json_helper.HIDEME_AVAILABLE_ENTITIES', ['H1', 'H2']):
