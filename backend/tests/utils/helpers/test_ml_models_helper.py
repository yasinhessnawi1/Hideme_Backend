...
class TestGetSpacyModel(unittest.TestCase):
    ...
    def test_get_spacy_model_already_installed(self, mock_log_info, mock_spacy_load):
        ...
        mock_log_info.assert_called_once_with(
            "[OK] Loaded spaCy model 'en_core_web_sm' successfully"
        )
    ...
    @patch("backend.app.utils.helpers.ml_models_helper.log_info")
    def test_get_spacy_model_not_installed(
        self, mock_log_info, mock_log_warning, mock_spacy_download, mock_spacy_load
    ):
        mock_model = MagicMock()
        ...
        mock_log_warning.assert_called_once_with(
            "[OK] spaCy model 'en_core_web_sm' not found. Downloading..."
        )

        mock_log_info.assert_called_once_with(
            "[OK] Downloaded and loaded spaCy model 'en_core_web_sm'"
        )
    ...
    def test_get_spacy_model_download_fails(
        self, mock_log_error, mock_log_warning, mock_spacy_download, mock_spacy_load
    ):
    ...
    def test_get_spacy_model_with_different_model(self, mock_log_info, mock_spacy_load):
        ...
        mock_log_info.assert_called_once_with(
            "[OK] Loaded spaCy model 'en_core_web_lg' successfully"
        )
...
class TestGetHfNerPipeline(unittest.TestCase):
    ...
    @patch(
        "backend.app.utils.helpers.ml_models_helper.AutoModelForTokenClassification.from_pretrained"
    )
    @patch("backend.app.utils.helpers.ml_models_helper.pipeline")
    @patch("backend.app.utils.helpers.ml_models_helper.log_info")
    def test_get_hf_ner_pipeline_success(
        self,
        mock_log_info,
        mock_pipeline,
        ...
        mock_tokenizer_from_pretrained.assert_called_once_with(
            "dslim/bert-base-NER", trust_remote_code=True
        )
        ...
        mock_log_info.assert_called_once_with(
            "[OK] Successfully loaded Hugging Face model from 'dslim/bert-base-NER'"
        )
    ...
    @patch("backend.app.utils.helpers.ml_models_helper.pipeline")
    @patch("backend.app.utils.helpers.ml_models_helper.log_info")
    def test_get_hf_ner_pipeline_with_custom_aggregation(
        self,
        mock_log_info,
        mock_pipeline,
        mock_model_from_pretrained,
        ...
        result = get_hf_ner_pipeline(
            "dslim/bert-base-NER", aggregation_strategy="first"
        )
    ...
    @patch("backend.app.utils.helpers.ml_models_helper.AutoTokenizer.from_pretrained")
    @patch("backend.app.utils.helpers.ml_models_helper.log_error")
    def test_get_hf_ner_pipeline_tokenizer_error(
        self, mock_log_error, mock_tokenizer_from_pretrained
    ):
        mock_tokenizer_from_pretrained.side_effect = Exception(
        ...
        mock_tokenizer_from_pretrained.assert_called_once_with(
            "invalid/model", trust_remote_code=True
        )

        mock_log_error.assert_called_once()
        self.assertIn(
            "Failed to load Hugging Face model", mock_log_error.call_args[0][0]
        )
    ...
    @patch("backend.app.utils.helpers.ml_models_helper.log_error")
    def test_get_hf_ner_pipeline_model_error(
        self, mock_log_error, mock_model_from_pretrained, mock_tokenizer_from_pretrained
    ):
        mock_tokenizer = MagicMock()
    ...
    @patch("backend.app.utils.helpers.ml_models_helper.pipeline")
    @patch("backend.app.utils.helpers.ml_models_helper.log_error")
    def test_get_hf_ner_pipeline_pipeline_error(
        self,
        mock_log_error,
        mock_pipeline,
        mock_model_from_pretrained,
    ...
    @patch("backend.app.utils.helpers.ml_models_helper.pipeline")
    @patch("backend.app.utils.helpers.ml_models_helper.log_info")
    def test_get_hf_ner_pipeline_local_path(
        self,
        mock_log_info,
        mock_pipeline,
        mock_model_from_pretrained,
        ...
        mock_log_info.assert_called_once_with(
            f"[OK] Successfully loaded Hugging Face model from '{local_path}'"
        )
