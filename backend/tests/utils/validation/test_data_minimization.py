...
class TestDataMinimization(unittest.TestCase):
    ...
    @patch(
        "backend.app.utils.constant.constant.SENSITIVE_FIELDS",
    ...
    @patch(
        "backend.app.utils.validation.data_minimization.SENSITIVE_FIELDS",
        ["sensitive", "confidence"],
    )
    ...
    @patch(
        "backend.app.utils.validation.data_minimization.DATA_MINIMIZATION_RULES",
        {"required_fields_only": True},
    )
    @patch(
        "backend.app.utils.validation.data_minimization.DEFAULT_METADATA_FIELDS",
    ...
    def test_minimize_extracted_data_positive(
        self,
    ...
    def test_remove_unwanted_fields_exception(self, mock_logger):
        ...
        mock_logger.warning.assert_called_once_with(
            "Error removing field '%s': %s", "bad", mock.ANY
        )
    ...
    @patch(
        "backend.app.utils.validation.data_minimization._apply_sensitive_patterns",
        return_value="ZZZ",
    )
    ...
    def test_sanitize_document_metadata_full(self, mock_record_keeper):
        ...
        out = sanitize_document_metadata(
            orig.copy(), sanitize_all=True, preserve_fields=["page_count"]
        )
