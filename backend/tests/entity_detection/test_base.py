...
class DummyDetector(BaseEntityDetector):
    ...
    async def detect_sensitive_data_async(
        self, extracted_data, requested_entities=None
    ):
        return [{"entity_type": "EMAIL", "start": 0, "end": 5, "score": 0.95}], {
            "pages": [{"sensitive": []}]
        }
...
class TestBaseEntityDetector(unittest.TestCase):
    ...
    def test_update_entity_metrics_fail_to_acquire_lock(self, mock_log, mock_lock):
        ...
        mock_log.assert_called_once_with(
            "Failed to acquire lock for updating entity statistics"
        )
    ...
    def test_update_usage_metrics_lock_fail(self, mock_log, mock_lock):
        ...
        mock_log.assert_called_once_with(
            "Failed to acquire lock for updating entity metrics"
        )

    # process_entities_for_page returns correct structure on success
    @patch.object(
        DummyDetector,
        "_standardize_raw_entities",
        return_value=[{"entity_type": "EMAIL", "start": 0, "end": 5, "score": 0.95}],
    )
    @patch.object(
        DummyDetector,
        "_process_sanitized_entities",
        return_value=(
            [{"entity_type": "EMAIL", "start": 0, "end": 5, "score": 0.95}],
            [{"entity_type": "EMAIL"}],
        ),
    )
    @patch.object(DummyDetector, "_update_entity_metrics")
    @patch.object(DummyDetector, "_record_entity_processing")
    def test_process_entities_for_page_success(
        self, mock_record, mock_update, mock_process, mock_standardize
    ):
        result = asyncio.run(
            self.detector.process_entities_for_page(
                page_number=1,
    ...
    def test_process_entities_standardize_failure(
        self, mock_process, mock_standardize, mock_error_log
    ):
        result = asyncio.run(
            self.detector.process_entities_for_page(1, "text", [], ["bad_entity"])
        )
    ...
    @patch.object(
        DummyDetector,
        "_process_sanitized_entities",
        side_effect=Exception("Sanitized Error"),
    )
    def test_process_entities_sanitized_processing_failure(
        self, mock_process, mock_standardize, mock_error_log
    ):
        result = asyncio.run(
            self.detector.process_entities_for_page(1, "text", [], ["entity"])
        )
    ...
    @patch.object(DummyDetector, "_process_sanitized_entities", return_value=([], []))
    @patch.object(
        DummyDetector, "_update_entity_metrics", side_effect=Exception("Update Error")
    )
    def test_process_entities_metric_update_failure(
        self, mock_update, mock_process, mock_standardize, mock_error_log
    ):
        result = asyncio.run(self.detector.process_entities_for_page(1, "text", [], []))
    ...
    def test_standardize_raw_entities_all_paths(self, mock_warning, mock_logger):
        ...
        with patch.object(
            DummyDetector, "_convert_to_entity_dict", side_effect=Exception("boom")
        ):
            with patch(
                "backend.app.utils.system_utils.error_handling.SecurityAwareErrorHandler.log_processing_error"
            ) as mock_log:
    ...
    def test_process_sanitized_entities_positive(self, mock_text_utils):
        ...
        processed, sensitive = self.detector._process_sanitized_entities(
            sanitized, "hello world", [("dummy", 0, 5)]
        )
    ...
    def test_process_sanitized_entities_exception(self, mock_text_utils, mock_log):
        ...
        with patch.object(
            self.detector, "_process_single_entity", side_effect=Exception("boom")
        ):
            processed, sensitive = self.detector._process_sanitized_entities(
                sanitized, "hello world", [("dummy", 0, 5)]
            )
    ...
    def test_process_single_entity_success(self, mock_text_utils):
        ...
        mock_text_utils.map_offsets_to_bboxes.return_value = [
            {"x": 0, "y": 0, "w": 5, "h": 5}
        ]

        mock_text_utils.merge_bounding_boxes.return_value = {
            "composite": {"x": 0, "y": 0, "w": 5, "h": 5}
        }
        ...
        processed, sensitive = self.detector._process_single_entity(
            entity, "hello world", [("dummy", 0, 5)]
        )
    ...
    @patch(
        "backend.app.entity_detection.base.TextUtils.recompute_offsets",
