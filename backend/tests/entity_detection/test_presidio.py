...
class TestPresidioEntityDetector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch(
            "backend.app.entity_detection.presidio.PresidioEntityDetector.__init__"
        ) as mock_init:
    ...
    @patch("backend.app.entity_detection.presidio.log_error")
    def test_initialize_presidio_success(
        self, mock_log_error, MockAnonymizerEngine, MockAnalyzerEngine
    ):
    ...
    @patch("backend.app.entity_detection.presidio.AnalyzerEngine")
    @patch("backend.app.entity_detection.presidio.AnonymizerEngine")
    def test_initialize_presidio_warning_missing_config(
        self, mock_anonymizer, mock_analyzer, mock_log_warning
    ):
    ...
    def test_validate_entities_invalid(self, mock_log_warning):
        ...
        result, redaction_mapping = asyncio.run(
            self.detector.detect_sensitive_data_async({}, invalid_entities)
        )
    ...
    @patch("backend.app.entity_detection.presidio.log_warning")
    @patch(
        "backend.app.entity_detection.presidio.PresidioEntityDetector._process_all_pages_async"
    )
    def test_process_single_page_async_invalid(
        self, mock_process_all_pages, mock_log_warning
    ):
        ...
        result = asyncio.run(
            self.detector.detect_sensitive_data_async(
                {"page": 1, "words": ["test"]}, ["email"]
            )
        )
    ...
    async def test_run_async_detector_timeout(self, mock_log_info):
        ...
        result = await self.detector._run_async_detector(
            MagicMock(spec=EntityDetector), {}, []
        )
    ...
    def test_detect_sensitive_data_async_validation_failure(
        ...
        mock_process_all_pages,
        mock_minimize_extracted_data,
    ...
    def test_detect_sensitive_data_async_no_valid_entities(
        ...
        mock_minimize_extracted_data.return_value = {
            "pages": [{"page": 1, "words": ["test"]}]
        }
    ...
    @patch("backend.app.entity_detection.presidio.log_warning")
    @patch(
        "backend.app.entity_detection.presidio.SecurityAwareErrorHandler.log_processing_error"
    )
    @patch(
        "backend.app.entity_detection.presidio.PresidioEntityDetector._get_analyzer_lock"
    )
    ...
    def test_analyze_text_lock_acquisition_failure(
        self, mock_get_lock, mock_log_warning
    ):
        ...
        mock_log_warning.assert_called_with(
            "[WARNING] Failed to acquire lock for Presidio analysis"
        )
    ...
    def test_analyze_text_analyzer_not_initialized(
        self, mock_get_lock, mock_log_warning
    ):
        ...
        mock_log_warning.assert_called_with(
            "[WARNING] Presidio analyzer not initialized, returning empty result"
        )
    ...
    def test_analyze_text_successful_analysis(
        self, mock_get_lock, mock_log_processing_error
    ):
    ...
    def test_analyze_text_analysis_exception(
        self, mock_get_lock, mock_log_processing_error
    ):
        ...
        mock_log_processing_error.assert_called_once_with(mock.ANY, "presidio_analysis")
    ...
    @patch(
        "backend.app.entity_detection.presidio.ParallelProcessingCore.process_pages_in_parallel"
    )
    @patch(
        "backend.app.entity_detection.presidio.PresidioEntityDetector._process_single_page_async"
    )
    async def test_process_all_pages_async_success(
        self, mock_process_single_page, mock_process_pages
    ):
        # _process_all_pages_async returns results for each page
        mock_process_single_page.return_value = (
            {"page": 1, "sensitive": ["email"]},
            ["email"],
        )
        ...
        self.assertEqual(
            result, [(1, ({"page": 1, "sensitive": ["email"]}, ["email"]))]
        )

        mock_process_single_page.assert_called_once_with(
            {"page": 1, "words": ["test text"]}
        )
    ...
    async def test_process_all_pages_async_timeout(
        self, mock_process_single_page, mock_process_pages
    ):
        # _process_all_pages_async handles page-level timeouts gracefully
        mock_process_pages.side_effect = asyncio.TimeoutError(
            "Timeout processing pages"
        )
    ...
    async def test_process_all_pages_async_exception(
        self, mock_log_processing_error, mock_process_single_page, mock_process_pages
    ):
        ...
        mock_log_processing_error.assert_called_with(
            mock.ANY, "presidio_page_processing", "page_1"
        )
    ...
    def test_aggregate_page_results_valid(self):
        ...
        combined_entities, redaction_mapping = (
            PresidioEntityDetector._aggregate_page_results(local_page_results)
        )
