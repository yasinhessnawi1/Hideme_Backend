...
class TestGeminiEntityDetector(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_api_lock_handles_exception(self, mock_log):
        with patch("backend.app.entity_detection.gemini.AsyncTimeoutLock", side_effect=Exception("fail")):
    ...
    @patch("backend.app.utils.security.processing_records.record_keeper.record_processing")
    @patch("backend.app.entity_detection.gemini.log_warning")
    async def test_detect_sensitive_data_async_no_requested_entities(self, log_warn, record):
    ...
    @patch("backend.app.utils.security.processing_records.record_keeper.record_processing")
    @patch("backend.app.entity_detection.gemini.log_warning")
    @patch("backend.app.utils.helpers.json_helper.validate_gemini_requested_entities", side_effect=Exception("bad"))
    async def test_detect_sensitive_data_async_validation_fails(self, validate, log_war, record):
        result = await self.detector.detect_sensitive_data_async({"pages": []}, ["EMAIL"])
    ...
    @patch("backend.app.entity_detection.gemini.log_warning")
    @patch("backend.app.utils.security.processing_records.record_keeper.record_processing")
    @patch("backend.app.utils.helpers.json_helper.validate_gemini_requested_entities", return_value=[])
    async def test_detect_sensitive_data_async_empty_after_validation(self, validate, record, warn):
        result = await self.detector.detect_sensitive_data_async({"pages": []}, ["EMAIL"])
    ...
    @patch("backend.app.entity_detection.gemini.GeminiEntityDetector._finalize_detection_results")
    @patch("backend.app.entity_detection.gemini.GeminiEntityDetector._process_all_pages_in_parallel",
           new_callable=AsyncMock)
    @patch("backend.app.entity_detection.gemini.GeminiEntityDetector._prepare_pages_and_mappings")
    @patch("backend.app.utils.helpers.json_helper.validate_gemini_requested_entities", return_value=["EMAIL-G"])
    async def test_detect_sensitive_data_async_success(self, mock_validate, mock_prepare, mock_process, mock_finalize):
        ...
        result = await self.detector.detect_sensitive_data_async(
            {"pages": [{"page": 1, "words": [{"text": "hello", "start": 0, "end": 5}]}]},
            ["EMAIL-G"]
        )
    ...
    @patch("backend.app.entity_detection.gemini.log_warning")
    @patch("backend.app.utils.security.processing_records.record_keeper.record_processing")
    @patch("backend.app.utils.helpers.json_helper.validate_gemini_requested_entities",
           side_effect=Exception("Forced validation failure"))
    async def test_detect_sensitive_data_async_exception(self, mock_validate, mock_record, mock_log_warning):
        result = await self.detector.detect_sensitive_data_async(
    ...
    def test_prepare_pages_and_mappings_handles_empty_and_valid(self):
        data = {"pages": [{"page": 1, "words": [{"text": "hello", "start": 0, "end": 5}]}, {"page": 2, "words": []}]}
    ...
    @patch("backend.app.utils.helpers.text_utils.TextUtils.reconstruct_text_and_mapping", side_effect=Exception("fail"))
    @patch("backend.app.utils.system_utils.error_handling.SecurityAwareErrorHandler.log_processing_error")
    ...
    @patch("backend.app.entity_detection.gemini.ParallelProcessingCore.process_pages_in_parallel",
           new_callable=AsyncMock)
    async def test_process_all_pages_in_parallel_success(self, mock_core):
        mock_core.return_value = [(1, ({"page": 1, "sensitive": []}, [{"entity_type": "EMAIL"}]))]

        redaction = {"pages": []}

        results = await self.detector._process_all_pages_in_parallel([{"page": 1}], {1: ("text", [])}, ["EMAIL"],
                                                                     redaction)
    ...
    def test_finalize_detection_results_logs_metrics(self):
        with patch("backend.app.entity_detection.gemini.log_info") as log:
            with patch("backend.app.utils.security.processing_records.record_keeper.record_processing"):
                with patch("backend.app.entity_detection.gemini.gemini_usage_manager.get_usage_summary",
    ...
    def test_extract_entities_from_response_success(self):
        response = {"pages": [{"text": [{"entities": [{"start": 0, "end": 5, "entity_type": "EMAIL"}]}]}]}
    ...
    def test_extract_entities_from_response_exception(self, mock_log):
        with patch.object(self.detector, "_gather_raw_entities", side_effect=Exception("fail")):
    ...
    def test_maybe_add_original_text_invalid(self):
        ...
        with patch("backend.app.utils.logging.logger.log_warning") as warn:
    ...
    @patch("backend.app.entity_detection.gemini.gemini_usage_manager.manage_page_processing", new_callable=AsyncMock)
    @patch("backend.app.entity_detection.gemini.gemini_usage_manager.release_request_slot", new_callable=AsyncMock)
    @patch("backend.app.utils.logging.logger.log_info")
    @patch("backend.app.utils.logging.logger.log_warning")
    @patch("backend.app.utils.system_utils.error_handling.SecurityAwareErrorHandler.log_processing_error")
    async def test_process_single_page_success(self, mock_error_log, mock_warning, mock_info, mock_release,
                                               mock_manage):
        mock_lock = MagicMock()
        ...
        self.detector._extract_entities_from_response = MagicMock(return_value=[{"start": 0, "end": 5}])

        self.detector.process_entities_for_page = AsyncMock(
            return_value=([{"entity_type": "EMAIL"}], {"page": 1, "sensitive": [{"entity_type": "EMAIL"}]}))
    ...
    async def test_process_single_page_empty_words(self):
        result = await self.detector._process_single_page({"page": 1, "words": []}, {}, ["EMAIL"])
